...
function x = Thomas_loopless(A, d)
  # dimension of system 
  n = size(A,1);
  ...
  dp(1, 1) = d(1, 1) / b(1, 1)
    # c' 
    cp = c(2:n-1, 1) ./ (b(2:n-1, 1) - a(2:n-1).*cp(1:n-2,1)  );
##  for i=2:n-1
##    cp(i, 1) = c(i, 1) / ( b(i, 1) - a(i, 1)*cp(i-1, 1) );
##  endfor
    # d' 
    dp = ( d(2:n, 1) - a(2:n, 1).*dp(1:n-1, 1)) ./ (b(2:n, 1) - a(2:n, 1).* cp(1:n-1, 1) ) ;
##  for i=2:n
...
##  endfor
  ...
  x = dp(n-1:-1:1, 1) - cp(n-1:-1:1, 1).* x(n:-1:2, 1);
 # for i = n-1:-1:1
